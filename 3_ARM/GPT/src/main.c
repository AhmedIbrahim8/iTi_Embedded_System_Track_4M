...
int main(){
	...
	MGPIO_SetMode(PORTA,PIN4,INPUT_RESET_STATE);
	MGPIO_SetInputPinMode(PORTA,PIN4,PULL_UP);

	MEXTI_voidInterruptSetPort(EXTI_LINE_4,PORTA_PIN);
	MEXTI_voidInterruptEnableDisable(EXTI_LINE_4,EXTI_ENABLE);
	MEXTI_voidInterruptTrigger(EXTI_LINE_4,EXTI_FALLING_EDGE);
	MEXTI_voidSetCallBack(EXTI_LINE_4,GetFrame);

	TIMER_VoidSetCallBack(TIMER2_SELECTION,GetData);

	MNVIC_VoidEnbleInterrupt(EXTINT_EXTI4_POSITION);
...
//	MGPIO_SetPinValue(PORTA,PIN4,PIN_LEVEL_LOW);
	...
	TIMER_SetARR(TIMER5_SELECTION,2000000);

	TIMER_SetCompareValue(TIMER5_SELECTION,TIMER_CHANNEL_1,500000);
	TIMER_SetCompareValue(TIMER5_SELECTION,TIMER_CHANNEL_2,1000000);
	TIMER_SetCompareValue(TIMER5_SELECTION,TIMER_CHANNEL_3,1250000);
	TIMER_SetCompareValue(TIMER5_SELECTION,TIMER_CHANNEL_4,1500000);
