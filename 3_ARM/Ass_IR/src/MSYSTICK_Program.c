...
{
	return (SYSTICK->STK_LOAD-SYSTICK->STK_VAL)/16;
}
...
void SysTick_Handler(){
	if(global_flag == 1)
	{
		CLEAR_BIT(SYSTICK->STK_CTRL,SYSTICK_TICKINT_BIT);
		SYSTICK->STK_LOAD=0;
		SYSTICK->STK_VAL=0;
		CLEAR_BIT(SYSTICK->STK_CTRL,SYSTICK_ENABLE_BIT);

	}
	(*global_ptr)();
	GET_BIT(SYSTICK->STK_CTRL,SYSTICK_COUNTFLAG_BIT);
}


void SYSTICK_SetInterval(u32 Copy_u32LoadValue)
{
	SET_BIT(SYSTICK->STK_CTRL,SYSTICK_TICKINT_BIT);
	SYSTICK_voidStart(Copy_u32LoadValue);
	global_flag=1;
}
