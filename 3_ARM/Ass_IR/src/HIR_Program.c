...
void GetFrame(void)
{
	...
	if(flag == HIR_NUMBER_0)
	{
		...
		SYSTICK_SetInterval(SYSTICK_TIME_START);
...
void GetData(void)
{
	...
	for(i=HIR_NUMBER_0;i<HIR_NUMBER_8;i++)
	{
		...
		if((ProtocolFrame[HIR_NUMBER_17+i]>=HIR_START_TIME_OF_LOGIC_0) && (ProtocolFrame[HIR_NUMBER_17+i]<=HIR_END_TIME_OF_LOGIC_0))
		{
			CLEAR_BIT(data,i);
		}
		/* Incase of the one bit */
		else if((ProtocolFrame[HIR_NUMBER_17+i]>=HIR_START_TIME_OF_LOGIC_1) && (ProtocolFrame[HIR_NUMBER_17+i]<=HIR_END_TIME_OF_LOGIC_1))
		{
			SET_BIT(data,i);
		}
	}
