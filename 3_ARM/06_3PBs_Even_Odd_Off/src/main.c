...
int main(){
	...
	MGPIO_SetInputPinMode(PORTB,PIN1,PULL_DOWN);

	/* PORTB Pin 0 is output pin */
	MGPIO_SetMode(PORTB,PIN2,INPUT_RESET_STATE);
	MGPIO_SetInputPinMode(PORTB,PIN2,PULL_DOWN);
	while(1)
	{
		MGPIO_GetPinValue(PORTB,PIN0,&pin_value1);
		MGPIO_GetPinValue(PORTB,PIN1,&pin_value2);
		MGPIO_GetPinValue(PORTB,PIN2,&pin_value3);
		...
		if(pin_value3==1 && flag3==1){

			value=0;
			MGPIO_SetPortValue(PORTA,0);
