...
int main(){
	...
	MGPIO_SetAlternativeFuncPin(PORTA,PIN3,AF1_TIM1_TIM2);

	TIMER_Init(&Timer_configuration);

	TIMER_SetPrescaler(TIMER2_SELECTION, 15);

	TIMER_SetDutyCycle(TIMER2_SELECTION, TIMER_CHANNEL_4 , 3);
	...
	while(1){
		for(counter = 600;counter<=2400;counter++){
			TIMER_SetCompareValue(TIMER2_SELECTION, TIMER_CHANNEL_4 ,  counter);
			SYSTICK_voidDelayMs(5);
		}
		for(counter = 2399;counter>600;counter--){
			TIMER_SetCompareValue(TIMER2_SELECTION, TIMER_CHANNEL_4 ,  counter);
			SYSTICK_voidDelayMs(5);
			}
//		for(counter = 3;counter<=12;counter++){
//			TIMER_SetDutyCycle(TIMER2_SELECTION, TIMER_CHANNEL_4 , counter);
//			SYSTICK_voidDelayMs(100);
//		}
//		for(counter = 11;counter>3;counter--){
//			TIMER_SetDutyCycle(TIMER2_SELECTION, TIMER_CHANNEL_4 , counter);
//			SYSTICK_voidDelayMs(100);
//			}
	}
