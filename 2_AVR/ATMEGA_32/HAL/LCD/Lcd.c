...
void LCD_voidInit(void){
	...
	for(counter=0;counter<LCD_NUMBERS;counter++){
		if(configuration[counter].mode==LCD_8_BIT_MODE)
		{
			...
			command=FUNCTION_SET_COMMAND|(1<<pinsConfiguration[counter].dataPins[LCD1_DB_4])|\
					(configuration[counter].line<<pinsConfiguration[counter].dataPins[LCD1_DB_3])|\
					(configuration[counter].font<<pinsConfiguration[counter].dataPins[LCD1_DB_2]);
			LCD_voidWriteCommand(counter,command);
			_delay_ms(1);
			/* Display On Off Control command*/
			command=DISPLAY_ON_OFF_COMMAND|(configuration[counter].display<<pinsConfiguration[counter].dataPins[LCD1_DB_2])|\
					(configuration[counter].cursor<<pinsConfiguration[counter].dataPins[LCD1_DB_1])|\
					(configuration[counter].blink<<pinsConfiguration[counter].dataPins[LCD1_DB_0]);
			LCD_voidWriteCommand(counter,command);
			_delay_ms(1);
			...
			_delay_ms(3);
			/* Entry mode command */
			command=ENTRY_MODE_COMMAND|(configuration[counter].cursorMove<<pinsConfiguration[counter].dataPins[LCD1_DB_1])|\
					(configuration[counter].shift<<pinsConfiguration[counter].dataPins[LCD1_DB_0]);
			LCD_voidWriteCommand(counter,command);
			_delay_ms(3);
		}
		else
		{
			...
			LCD_voidWriteCommand(counter,FUNCTION_SET_COMMAND);
			command=FUNCTION_SET_COMMAND|(configuration[counter].line<<pinsConfiguration[counter].dataPins[LCD1_DB_3])|\
					(configuration[counter].font<<pinsConfiguration[counter].dataPins[LCD1_DB_2]);
			command=command<<4;
			LCD_voidWriteCommand(counter,command);
			_delay_ms(1);
